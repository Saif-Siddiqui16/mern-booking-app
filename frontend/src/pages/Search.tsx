...
const Search = () => {
  ...
  const handleHotelChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const hotelChange = event.target.value;
    setSelectedHotelTypes((prevHotel) =>
      event.target.checked
        ? [...prevHotel, hotelChange]
        : prevHotel.filter((hotel) => hotel !== hotelChange)
    );
  };

  const handleFacilityChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const facilityChange = event.target.value;
    setSelectedFacilities((prevFacility) =>
      event.target.checked
        ? [...prevFacility, facilityChange]
        : prevFacility.filter((facility) => facility !== facilityChange)
    );
  };
